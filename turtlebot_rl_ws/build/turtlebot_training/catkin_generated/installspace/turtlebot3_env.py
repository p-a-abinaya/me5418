...
class TurtleBot3Env(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self): 
        ...
        * /camera/depth/points : point cloud data
        ...
        #rospy.Subscriber("/camera/depth/points", PointCloud2, self._depth_cloud_callback)
        # subscriber for /elevation_mapping/elevation_map
        rospy.Subscriber("/elevation_mapping/elevation_map", GridMap, self._elevation_map_callback)
    ...
    def _check_elevation_map_ready(self):
        self.elevation_map = None
        rospy.logdebug("Waiting for /camera/depth/points to be READY...")
        while self.elevation_map is None and not rospy.is_shutdown():
            try:
                self.elevation_map = rospy.wait_for_message("/elevation_mapping/elevation_map", GridMap, timeout=5.0)
                rospy.logdebug("Current /elevation_mapping/elevation_map ready=>")

            except:
                rospy.logerr("Current /elevation_mapping/elevation_map not ready yet, retrying")
    ...
    def _elevation_map_callback(self, data):
